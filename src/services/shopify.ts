...
const MOCK_PRODUCTS: ShopifyProduct[] = [
    ...
    images: [
      {
        ...
        altText: "Stiletto Silk Slip Dress front view",
      },
      {
        url: "https://images.unsplash.com/photo-1485462537746-965f33f7f6a7?w=800&q=80",
        altText: "Stiletto Silk Slip Dress detail",
      },
    ],
    options: [
      ...
      {
        name: "Color",
        values: ["Champagne", "Midnight Black", "Emerald Green"],
      },
    ],
    variants: [
      {
  ...
  },
  {
    ...
    description:
      "A relaxed-fit utility shirt made from certified organic Belgian flax linen. Pre-washed for exceptional softness and styled with two chest utility pockets, button cuffs, and a classic collar. Light, breathable, and perfect for hot summer days.",
    ...
    images: [
      {
        ...
        altText: "Classic Linen Utility Shirt",
      },
    ],
    options: [
  ...
  },
  {
    ...
    description:
      "Handcrafted in Portugal, the Metro Chelsea Boots feature a premium full-grain Italian calfskin upper, a durable stacked leather sole with rubber grip overlays, and elastic side panels for easy entry. Classic style reimagined for modern commuters.",
    ...
    images: [
      {
        ...
        altText: "Metro Leather Chelsea Boots",
      },
    ],
    options: [
  ...
  },
  {
    ...
    description:
      "Cut from heavy 14oz Japanese selvedge denim. This jacket is unwashed and raw, designed to break in and develop custom creases and fades unique to your lifestyle. Designed with an oversized drop-shoulder silhouette.",
    ...
    images: [
      {
        ...
        altText: "Oversized Raw Denim Jacket",
      },
    ],
  ...
  },
  {
    ...
    description:
      "Classic hand-polished acetate sunglasses in a timeless tortoise shell pattern. Outfitted with Category 3 CR-39 polarized lenses offering 100% UVA/UVB protection and anti-scratch coatings.",
    ...
    images: [
      {
        ...
        altText: "Polarized Tortoise Sunglasses",
      },
    ],
    options: [{ name: "Color", values: ["Tortoise Shell", "Matte Black"] }],
    variants: [
      {
        id: "var-5-1",
  ...
  },
  {
    ...
    description:
      "Dainty everyday thick hoops crafted in 18k gold vermeil over sterling silver. Extremely lightweight, waterproof, hypoallergenic, and finished with secure click-on hinges.",
    ...
    images: [
      {
        ...
        altText: "Minimalist Gold Hoop Earrings",
      },
    ],
    options: [{ name: "Material", values: ["18k Gold", "925 Silver"] }],
    variants: [
      {
        id: "var-6-1",
        title: "18k Gold",
...
const MOCK_COLLECTIONS: ShopifyCollection[] = [
  {
    ...
    description:
      "Lightweight fabrics, sun-drenched styles, and seasonal accessories.",
    image: {
      url: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=800&q=80",
      altText: "Summer Essentials Collection Banner",
    },
  ...
  },
  {
    ...
    handle: "new-arrivals",
    description:
      "Be the first to shop the latest silhouettes and trendsetting drops.",
    image: {
  ...
  },
  {
    ...
    image: {
      url: "https://images.unsplash.com/photo-1479064555552-3ef4979f8908?w=800&q=80",
      altText: "Best Sellers Collection Banner",
    },
    products: [
      MOCK_PRODUCTS[0],
      MOCK_PRODUCTS[2],
...
export async function getProducts(): Promise<ShopifyProduct[]> {
  if (SHOPIFY_DOMAIN && STOREFRONT_ACCESS_TOKEN) {
    try {
      ...
      `;
      const data = await storefrontQuery<{ products: any }>(query);
      if (data?.products) {
    ...
    try {
      ...
      if (data?.collection) {
        ...
        return {
          ...
          image: c.image
            ? { url: c.image.url, altText: c.image.altText }
            : undefined,
...
export async function getCollections(
  ...
  if (SHOPIFY_DOMAIN && STOREFRONT_ACCESS_TOKEN) {
    try {
      ...
      if (data?.collections) {
        const results: ShopifyCollection[] = data.collections.edges.map(
          (e: any) => {
            const c = e.node;
            return {
              id: c.id,
        ...
        if (ids && ids.length > 0) {
          return results.filter(
            (r) => ids.includes(r.id) || ids.includes(r.handle),
          );
        }
        return results;
      }
  ...
  if (ids && ids.length > 0) {
    return MOCK_COLLECTIONS.filter(
      (c) => ids.includes(c.id) || ids.includes(c.handle),
    );
  }
...
export async function searchProducts(query: string): Promise<ShopifyProduct[]> {
  ...
  if (SHOPIFY_DOMAIN && STOREFRONT_ACCESS_TOKEN) {
    try {
      ...
      `;
      const data = await storefrontQuery<{ products: any }>(gqlQuery, {
        query,
      });
      if (data?.products) {
        return data.products.edges.map((e: any) => {
      ...
      console.warn("GraphQL search error, falling back to mock", e);
    }
  }

  // Mock fallback: simple title/description string match
...
export async function getRelatedProducts(
  ...
  // For live Shopify: use productRecommendations query
  if (SHOPIFY_DOMAIN && STOREFRONT_ACCESS_TOKEN) {
    try {
      // First resolve handle → id
      const idQuery = `
        query getProductId($handle: String!) {
          productByHandle(handle: $handle) { id }
        }
      `;
      const idData = await storefrontQuery<{
        productByHandle: { id: string } | null;
      }>(idQuery, { handle: productHandle });
      const productId = idData?.productByHandle?.id;
      if (productId) {
        const recQuery = `
          query productRecommendations($productId: ID!) {
            productRecommendations(productId: $productId) {
              id title description handle availableForSale vendor
        ...
        `;
        const recData = await storefrontQuery<{
          productRecommendations: any[];
        }>(recQuery, {
          productId,
        });
        if (recData?.productRecommendations?.length) {
          return recData.productRecommendations
            .slice(0, limit)
            .map((p: any) => ({
              id: p.id,
              title: p.title,
      ...
      console.warn("Related products GraphQL error, falling back to mock", e);
    }
  }

  // Mock: return products excluding the current one
  return MOCK_PRODUCTS.filter((p) => p.handle !== productHandle).slice(
    0,
    limit,
  );
}

// ─── Collection Products with Sort & Filter ──────────────────────────────────
...
export async function getCollectionProducts(
  ...
  const { first = 20, after, sortKey = "RELEVANCE", filters } = options;
  ...
  if (SHOPIFY_DOMAIN && STOREFRONT_ACCESS_TOKEN) {
    try {
      ...
      `;
      const data = await storefrontQuery<{ collectionByHandle: any }>(query, {
      ...
      if (col) {
        const products: ShopifyProduct[] = col.products.edges.map((e: any) => {
          const p = e.node;
          return {
            id: p.id,
            title: p.title,
      ...
      console.warn(
        "Collection products GraphQL error, falling back to mock",
        e,
      );
    }
  }
  ...
  // Mock fallback
  const col = MOCK_COLLECTIONS.find((c) => c.handle === collectionHandle);
  let products = col?.products ?? MOCK_PRODUCTS;

  if (filters?.available) products = products.filter((p) => p.availableForSale);
  if (filters?.vendor)
    products = products.filter((p) => p.vendor === filters.vendor);
  if (filters?.minPrice)
    products = products.filter(
      (p) =>
        parseFloat(p.priceRange.minVariantPrice.amount) >= filters.minPrice!,
    );
  if (filters?.maxPrice)
    products = products.filter(
      (p) =>
  ...
  if (sortKey === "PRICE" || sortKey === "PRICE_DESC") {
    products = [...products].sort((a, b) => {
      const pa = parseFloat(a.priceRange.minVariantPrice.amount);
      const pb = parseFloat(b.priceRange.minVariantPrice.amount);
      return sortKey === "PRICE" ? pa - pb : pb - pa;
    });
  }

  return { products: products.slice(0, first), hasNextPage: false };
}
